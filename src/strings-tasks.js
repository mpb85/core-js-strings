...
function removeFirstOccurrences(str, value) {
  ...
  const newStr = str.slice(0, strIndex) + str.slice(strIndex + value.length);
  return newStr;
}
...
function removeLastOccurrences(str, value) {
  const strLastIndex = str.lastIndexOf(value);
  if (strLastIndex === -1) return str;
  const newStr =
    str.slice(0, strLastIndex) + str.slice(strLastIndex + value.length);
  return newStr;
}
...
function findLongestWord(sentence) {
  ...
  if (arr.length === 0) return sentence;
  const longestWorld = arr.sort((a, b) => b.length - a.length);
  return longestWorld[0];
}
...
function extractNameFromTemplate(value) {
  ...
  return value.slice(firstIndex + 1, lastIndex);
}
...
function encodeToRot13(str) {
  ...
  const outputStrArr = outputStr.split('');
  const newArr = strArr.map((char) => outputStrArr[inputStrArr.indexOf(char)]);
  return newArr.join('');
}
